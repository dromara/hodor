...
CREATE TABLE `hodor_job_info` (
    ...
    `job_category` varchar(64) NOT NULL DEFAULT 'default' COMMENT '任务分类',
    `job_type` int(11) NOT NULL DEFAULT 0 COMMENT '任务命令类型， 0：普通类型 2：工作流类型 3：分片任务 3: 广播任务, 4：mr任务，默认0',
    `job_command_type` varchar(255) NOT NULL DEFAULT '' COMMENT '任务指令类型，java,shell,python',
    `job_command` varchar(255) NOT NULL DEFAULT '' COMMENT '任务执行的命令，Java任务即执行的方法',
    `schedule_strategy` int(11) NOT NULL DEFAULT 0 COMMENT '调度策略, 0: random, 1: round_robin, 2: lowest_load, 4: specify_actuator',
    `schedule_exp` varchar(64) NOT NULL DEFAULT '-' COMMENT '调度策略表达式',
    `time_type` int(11) NOT NULL DEFAULT 0 COMMENT '任务运行时间类型, 0: none, 1: cron, 2: fixed_delay, 3: fixed_rate, 4: once_time',
    `time_exp` varchar(64) NOT NULL DEFAULT '-' COMMENT '任务运行时间表达式',
    `time_zone` varchar(64) NOT NULL DEFAULT '' COMMENT '任务时区表达式',
    `job_parameters` varchar(512) NOT NULL DEFAULT '' COMMENT '分片参数',
    `extensible_parameters` varchar(512) NOT NULL DEFAULT '' COMMENT '可扩展任务参数，用于针对不是常规任务的扩展',
    ...
    `job_data_path` varchar(255) NOT NULL DEFAULT '' COMMENT '任务的jar，sql文件，sh文件信息等',
...
CREATE TABLE `hodor_job_exec_detail` (
    `id` bigint NOT NULL COMMENT '任务执行唯一标识',
    ...
    `scheduler_endpoint` char(20) DEFAULT NULL COMMENT '调度服务器IP',
    `actuator_endpoint` char(20) DEFAULT NULL COMMENT '业务执行服务器IP',
