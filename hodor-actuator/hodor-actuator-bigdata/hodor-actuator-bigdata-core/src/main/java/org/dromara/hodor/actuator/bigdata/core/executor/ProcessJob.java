...
import org.dromara.hodor.actuator.bigdata.core.executor.process.ExecutorProcess;
import org.dromara.hodor.actuator.bigdata.core.executor.process.ExecutorProcessBuilder;
import org.dromara.hodor.actuator.bigdata.core.utils.Pair;
...
public class ProcessJob extends AbstractProcessJob {
  ...
  private volatile ExecutorProcess process;
  ...
  public void run() throws Exception {
    ...
    //Get list of users we never execute flows as. (ie: root, hodor)
    final Set<String> blackListedUsers = new HashSet<>(
        Arrays.asList(
            this.sysProps.getString(Constants.ConfigurationKeys.BLACK_LISTED_USERS, "root,hodor")
                .split(",")
        )
    );
    ...
    */
    ...
    for (String command : commands) {
      ExecutorProcessBuilder builder = null;
      if (isExecuteAsUser) {
        command =
        ...
        builder =
            new ExecutorProcessBuilder(partitionCommandLine(command))
      ...
      synchronized (this) {
        // Make sure that checking if the process job is killed and creating an EjobExecutorProcess
  ...
  public void cancel() throws InterruptedException {
    ...
    final boolean processkilled = this.process
        .softKill(KILL_TIME.getMillis(), TimeUnit.MILLISECONDS);
    if (!processkilled) {
