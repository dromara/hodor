...
import static org.dromara.hodor.actuator.jobtype.bigdata.security.commons.SecurityUtils.ENABLE_PROXYING;
import static org.dromara.hodor.actuator.jobtype.bigdata.security.commons.SecurityUtils.OBTAIN_BINARY_TOKEN;
...
public class HadoopSparkJob extends JavaProcessJob {
  ...
  private static String testableGetMainArguments(final Props jobProps, final String cwd,
                                                 final String workingDir, final Logger log) {
    ...
    for (final SparkJobArg sparkJobArg : SparkJobArg.values()) {
      if (!sparkJobArg.needSpecialTreatment) {
        ...
        executionJarHelper(jobProps, cwd, workingDir, log, argList);
  ...
  private static void executionJarHelper(final Props jobProps, final String cwd,
    ...
    if (jobProps.containsKey(SparkJobArg.EXECUTION_JAR.azPropName)) {
      final String executionJarName =
          HadoopJobUtils.resolveExecutionJarName(cwd, workingDir,
  ...
  protected String getMainArguments() {
    ...
    return testableGetMainArguments(this.jobProps, getCwd(), getWorkingDirectory(), getLog());
  }
  ...
  protected List<String> getClassPaths() {
    ...
    classPath.add(getSourcePathFromClass(HadoopSecureSparkWrapper.class));
