...
/**
 * AbstractNetChannelHandler
 *
 * @author xiaoyu
 * @author tomgs
 */
public abstract class AbstractNetChannelHandler implements HodorChannelHandler {

    private final HodorChannelHandler handler;
     ...
     */
    public AbstractNetChannelHandler(Attribute attribute, HodorChannelHandler handler) {
        ...
        host = attribute.getProperty(RemotingConst.HOST_KEY);
        if (StringUtils.isBlank(host)) {
            throw new IllegalArgumentException("server host must not be null.");
        }

        port = attribute.getProperty(RemotingConst.PORT_KEY, -1);
        if (port <= 0 || port > 65535) {
            throw new IllegalArgumentException("server port is illegal argument, port <= 0 or port > 65535.");
        }
    }
