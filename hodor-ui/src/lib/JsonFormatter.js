export const JsonFormatter = function (opt) {
...
export const markDiffPrice = function (obj, prop, numberHtml) {
    try {
        if ((typeof compareResult) != 'undefined') {
            for (var vwhCode in compareResult) {
                if ('displayPrice' == prop) {
                    console.log(JSON.stringify(compareResult));
                    var vwh = obj.vwhCode;
                    if (vwh == vwhCode && compareResult[vwhCode] == obj[prop]) {
                        numberHtml = '<span style="font-weight: bold;color: red;">' + obj[prop] + '<span class="jf-Comma">,</span></span>';
                    }
                }
            }
        }
        if ((typeof appCompareResult) != 'undefined') {
            for (var vwhCode in appCompareResult) {
                if ('appPrice' == prop) {
                    console.log(JSON.stringify(appCompareResult));
                    var vwh = obj.vwhCode;
                    if (vwh == vwhCode && compareResult[vwhCode] == obj[prop]) {
                        numberHtml = '<span style="font-weight: bold;color: red;">' + obj[prop] + '<span class="jf-Comma">,</span></span>';
                    }
