...
// 编辑行
const editJobInfo = record => {
  console.log(record)
  openUpdateModal.value = true;
  editableData.value = cloneDeep(...jobInfoList.value.filter(item => record.id === item.id));
...
const getJobStatusDetail = (groupName, jobName) => {
  ...
  router.push({path: 'jobStatus', query: {groupName, jobName}});
}
...
<template>
  <div>
    <page-header
      title="任务信息管理"
      describe="展示任务信息，管理所有任务，具有新增、编辑、删除、暂停、恢复任务、查看任务执行详情等功能"
    />
    <page-layout>
      <a-card>
        <a-space direction="vertical">
          ...
          >
            <a-space>
              ...
              <a-modal
                v-model:visible="openCreateModal"
                ok-text="确定"
                cancel-text="取消"
                width="80%"
                    ...
                    >
                      <a-row :gutter="24">
                        <a-col :span="6">
                          ...
                          >
                            <a-select
                              ref="select"
                              v-model:value="formStateCreateJob.groupName"
                      ...
                      <a-row :gutter="24">
                        <a-col :span="12">
                          ...
                          >
                            <a-textarea
                              v-model:value="formStateCreateJob.jobDesc"
                              placeholder="请输入任务描述"
                              :rows="2"
                            />
                          </a-form-item>
                      ...
                      <a-row :gutter="24">
                        <a-col :span="5">
                          ...
                          >
                            <a-select
                              ref="select"
                              v-model:value="formStateCreateJob.jobType"
                            >
                              <a-select-option value="COMMON_JOB"/>
                              <a-select-option value="WORKFLOW_JOB"/>
                              <a-select-option value="SHARDING_JOB"/>
                              <a-select-option value="BROADCAST_JOB"/>
                              <a-select-option value="MAPREDUCE_JOB"/>
                            </a-select>
                          </a-form-item>
                        </a-col>
                        <a-col :span="5">
                          ...
                          >
                            <a-select
                              ref="select"
                              v-model:value="formStateCreateJob.timeType"
                            >
                              <a-select-option value="NONE"/>
                              <a-select-option value="CRON"/>
                              <a-select-option value="FIXED_RATE"/>
                              <a-select-option value="FIXED_DELAY"/>
                              <a-select-option value="ONCE_TIME"/>
                            </a-select>
                          </a-form-item>
                        </a-col>
                        <div
                          v-show="formStateCreateJob.timeType !== 'NONE'"
                          class="flexDisplay"
                        >
                          <a-col
                            :span="8"
                          ...
                          <a-col :span="4">
                            <a-button @click="onClickValidCron">
                              验证表达式
                            </a-button>
                          </a-col>
                        </div>
                      ...
                      <a-row :gutter="24">
                        <a-col :span="5">
                          ...
                          >
                            <a-select
                              ref="select"
                              v-model:value="formStateCreateJob.jobCommandType"
                            >
                              <a-select-option
                                v-for="item in jobTypeNames[bindingList[formStateCreateJob.groupName]]"
                                :value="item"
                              />
                      ...
                      <a-row :gutter="24">
                        <a-col :span="5">
                          ...
                          >
                            <a-select
                              ref="select"
                              v-model:value="formStateCreateJob.scheduleStrategy"
                            >
                              <a-select-option value="RANDOM"/>
                              <a-select-option value="ROUND_ROBIN"/>
                              <a-select-option value="LOWEST_LOAD"/>
                              <a-select-option value="SPECIFY"/>
                            </a-select>
                          </a-form-item>
                        </a-col>
                        <div
                          v-show="formStateCreateJob.scheduleStrategy === 'SPECIFY'"
                          class="flexDisplay"
                        >
                          <a-col
                            :span="12"
                      ...
                      <a-row :gutter="24">
                        <a-col :span="24">
                          ...
                          >
                            <codemirror
                              ref="codeMirrorRef"
                              v-model="formStateCreateJob.jobParameters"
                              :style="{ height: '100px' }"
                              :tab-size="2"
                              @ready="handleReady"
                            />
                      ...
                      <a-row :gutter="24">
                        ...
                        <a-col :span="12">
                          ...
                          >
                            <a-range-picker
                              v-model="formStateCreateJob.executeTime"
                      ...
                      <a-row :gutter="24">
                        <a-col :span="5">
                          ...
                          >
                            <a-select
                              ref="select"
                              v-model:value="formStateCreateJob.priority"
                            >
                        ...
                        <a-col :span="9">
                          <a-form-item name="checkValue">
                            <a-checkbox-group
                              v-model:value="formStateCreateJob.checkValue"
                              name="checkboxgroup"
                      ...
                      >
                        <a-space>
                          <a-button @click="openCreateModal = false">
                            取消
                          </a-button>
                          <a-button
                            type="primary"
                    ...
                    >
                      ...
                      >
                        <a-space>
                          ...
                          <a-button
                            type="primary"
                            @click="copyText(formEditor.content)"
                          >
                            复制
                          </a-button>
                        </a-space>
              ...
              <a-button
                type="primary"
                danger
                @click="onClickBtn(actionType.stop)"
              >
                暂停任务
              </a-button>
              <a-button
                type="primary"
                danger
                @click="onClickBtn(actionType.resume)"
              >
                恢复任务
              </a-button>
              <a-button
                type="primary"
                danger
                @click="onClickBtn(actionType.delete)"
              >
                删除任务
              </a-button>
            </a-space>
            ...
            >
              <a-space style="width: 100%">
                <a-form-item name="groupName">
                  <a-input
                    v-model:value="formStateSearch.groupName"
                    placeholder="任务组"
                  />
                </a-form-item>
                <a-form-item name="jobName">
                  <a-input
                    v-model:value="formStateSearch.jobName"
                    placeholder="任务名称"
                  />
                </a-form-item>
                <a-form-item name="jobStatus">
                  <a-select
                    ref="select"
                    v-model:value="formStateSearch.jobStatus"
                    placeholder="任务状态"
                  >
                ...
                <a-form-item name="jobType">
                  <a-select
                    ref="select"
                    v-model:value="formStateSearch.jobType"
                    placeholder="任务类型"
                  >
                    <a-select-option value="ALL"/>
                    <a-select-option value="COMMON_JOB"/>
            ...
            <a-space>
              ...
              <a-tooltip title="立即执行">
                <a-button
                  ...
                  @click="onClickBtn(actionType.execute, record)"
                >
                  <template #icon>
                    <ThunderboltTwoTone/>
                  </template>
              ...
              <a-tooltip title="恢复">
                <a-button
                  ...
                  @click="onClickBtn(actionType.resume, record)"
                >
                  <template #icon>
                    <RedoOutlined style="color: #2d8cf0"/>
                  </template>
                </a-button>
              </a-tooltip>
              <a-tooltip title="停止">
                <a-button
                  ...
                  @click="onClickBtn(actionType.stop, record)"
                >
                  <template #icon>
                    <StopOutlined style="color: #2d8cf0"/>
                  </template>
                </a-button>
              </a-tooltip>
              <a-tooltip title="任务详情">
                <a-button
                  ...
                  @click="getJobStatusDetail(record.groupName, record.jobName)"
                >
                  <template #icon>
                    <BarsOutlined style="color: #2d8cf0"/>
                  </template>
                </a-button>
              </a-tooltip>
            </a-space>
      ...
      <!-- 任务编辑页 -->
      <a-modal
        v-model:visible="openUpdateModal"
        ok-text="确定"
        cancel-text="取消"
        width="80%"
        @ok="saveJobInfo"
        ...
        >
          <a-row :gutter="24">
            <a-col :span="6">
              ...
              >
                <a-select
                  ref="select"
                  v-model:value="formStateUpdateJob.groupName"
                  :options="groupNameOptions"
                />
              </a-form-item>
            </a-col>
          ...
          <a-row :gutter="24">
            <a-col :span="12">
              ...
              >
                <a-textarea
                  v-model:value="formStateUpdateJob.jobDesc"
                  placeholder="请输入任务描述"
                  :rows="2"
                />
              </a-form-item>
          ...
          <a-row :gutter="24">
            <a-col :span="5">
              ...
              >
                <a-select
                  ref="select"
                  v-model:value="formStateUpdateJob.jobType"
                >
                  <a-select-option value="COMMON_JOB"/>
                  <a-select-option value="WORKFLOW_JOB"/>
                  <a-select-option value="SHARDING_JOB"/>
            ...
            <a-col :span="5">
              ...
              >
                <a-select
                  ref="select"
                  v-model:value="formStateUpdateJob.timeType"
                >
                  <a-select-option value="NONE"/>
                  <a-select-option value="CRON"/>
                  <a-select-option value="FIXED_RATE"/>
            ...
            <div
              v-show="formStateUpdateJob.timeType !== 'NONE'"
              class="flexDisplay"
            >
              <a-col
                :span="8"
          ...
          <a-row :gutter="24">
            <a-col :span="5">
              ...
              >
                <a-select
                  ref="select"
                  v-model:value="formStateUpdateJob.jobCommandType"
                >
                  <a-select-option
                    v-for="item in jobTypeNames[bindingList[formStateUpdateJob.groupName]]"
                    :value="item"
                  />
                  <!-- <a-select-option value="java"></a-select-option> -->
                </a-select>
          ...
          <a-row :gutter="24">
            <a-col :span="5">
              ...
              >
                <a-select
                  ref="select"
                  v-model:value="formStateUpdateJob.scheduleStrategy"
                >
                  <a-select-option value="RANDOM"/>
                  <a-select-option value="ROUND_ROBIN"/>
                  <a-select-option value="LOWEST_LOAD"/>
            ...
            <div
              v-show="formStateUpdateJob.scheduleStrategy === 'SPECIFY'"
              class="flexDisplay"
            >
              <a-col
                :span="12"
          ...
          <a-row :gutter="24">
            <a-col :span="24">
              ...
              >
                <codemirror
                  ref="codeMirrorRef"
                  v-model="formStateUpdateJob.jobParameters"
                  :style="{ height: '100px' }"
                  :tab-size="2"
                  @ready="handleReady"
                />
          ...
          <a-row :gutter="24">
            ...
            <a-col :span="12">
              ...
              >
                <a-range-picker
                  v-model="formStateUpdateJob.executeTime"
                  :show-time="{ format: 'HH:mm:ss' }"
                  format="YYYY-MM-DD HH:mm:ss"
                  allow-clear
                  show-now
          ...
          <a-row :gutter="24">
            <a-col :span="5">
              ...
              >
                <a-select
                  ref="select"
                  v-model:value="formStateUpdateJob.priority"
                >
                  <a-select-option value="DEFAULT"/>
                  <a-select-option value="MEDIUM"/>
                  <a-select-option value="HIGHER"/>
            ...
            <a-col :span="9">
              <a-form-item name="checkValue">
                <a-checkbox-group
                  v-model:value="formStateUpdateJob.checkValue"
                  name="checkboxgroup"
                  :options="checkboxOptions"
                />
              </a-form-item>
      ...
      <a-modal
        ...
        :title="`${eventType.value}任务`"
        @ok="handleOk"
        @cancel="handleCancel"
      >
        <p>确认{{ eventType }}下列任务吗？</p>
        <a-table
          v-show="selectedJobInfos.length > 0"
          :columns="selectedJobInfosColumns"
          :data-source="selectedJobInfos"
          :pagination="false"
        />
      </a-modal>
    </page-layout>
